...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        self.Q[state][action] = (
            self.Q[state][action]
            + self.learning_rate * (reward + self.discount_factor * self.Q[next_state][next_action] - self.Q[state][action])
        )
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        ...
        # ==> Calculate the utility of the state by choosing the action with the maximum q-value
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        self.Q[state][action] = (
            self.Q[state][action]
            + self.learning_rate * (reward + self.discount_factor * self.compute_utility(env, next_state) - self.Q[state][action])
        )
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # ==> if the next_state is a terminal state, then the utility of the state is 0
        # ==> else, calculate the utility of the next_state
        next_state_utility = 0 if done else self.__compute_utility_from_features(self.feature_extractor.extract_features(env, next_state))

        # ==> calculate the q-value of the state
        state_q_value = self.compute_q(env, state, action)

        # ==> create temp to store the weights
        temp = self.weights[action]
        ...
        for feature, value in self.feature_extractor.extract_features(env, state).items():
            temp[feature] = temp[feature] + self.learning_rate * (reward + self.discount_factor * next_state_utility - state_q_value) * value

        # ==> update the weights
        self.weights[action] = temp
