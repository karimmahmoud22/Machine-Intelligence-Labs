...
def forward_checking(problem: Problem, assigned_variable: str, assigned_value: Any, domains: Dict[str, set]) -> bool:
    ...
    for constraint in problem.constraints:
        ...
        if isinstance(constraint, BinaryConstraint) and assigned_variable in constraint.variables:
            ...
            new_domain = set(domains[other_variable])
            
            # Update the domain based on the binary constraint
            new_domain = {value for value in new_domain if constraint.condition(assigned_value, value)}
            domains[other_variable] = new_domain
            
            # If the domain becomes empty, the assignment is not consistent
            if not new_domain:
                return False
